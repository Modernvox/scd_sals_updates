...
ASYNC_MODE = "threading"
app = None  # Ensure app is defined at module level for Gunicorn
...
# --- Initialization block that always sets `app` ---
try:
    cfg = load_config()
    ...
    app = server.app  # ✅ Always defined for Gunicorn

except Exception as e:
    logging.getLogger(__name__).exception(f"Flask server failed to initialize: {e}")
    app = Flask(__name__)  # fallback dummy app
    @app.route("/")
    def init_error():
        return f"Server failed to initialize: {str(e)}", 500
