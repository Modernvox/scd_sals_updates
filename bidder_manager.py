...
# ─── CONFIGURATION ─────────────────────────────────────────────────────────────────
def get_default_bidders_db_path():
    """
    Returns a file path for bidders.db under the user’s local app data directory.
    """
    base_dir = os.getenv('LOCALAPPDATA', os.path.expanduser("~"))
    app_dir = os.path.join(base_dir, 'SwiftSaleApp')
    os.makedirs(app_dir, exist_ok=True)
    return os.path.join(app_dir, 'bidders.db')


def get_default_subscriptions_db_path():
    """
    Returns a file path for subscriptions.db under the user’s local app data directory.
    """
    base_dir = os.getenv('LOCALAPPDATA', os.path.expanduser("~"))
    app_dir = os.path.join(base_dir, 'SwiftSaleApp')
    os.makedirs(app_dir, exist_ok=True)
...
class BidderManager:
    """
    Manages bidder transactions and bin assignments for SwiftSale,
    using bidders.db for transactions and subscriptions.db for subscription tiers.
    """

    def __init__(self, bidders_db_path=None, subs_db_path=None):
        # ─── bidders.db ──────────────────────────────────────────────────────────
        self.bidders_db_path = bidders_db_path or get_default_bidders_db_path()
        ...
        # ─── subscriptions.db ─────────────────────────────────────────────────────
        self.subs_db_path = subs_db_path or get_default_subscriptions_db_path()
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    # Section: bidders.db schema creation / verification
    # ──────────────────────────────────────────────────────────────────────────────

    def _initialize_bidders_tables(self):
        """
        Create or verify the tables in bidders.db:
          - bidders_old(id, username, original_username, quantity, weight, is_giveaway, bin_number, giveaway_number, timestamp)
          - bin_assignments(username, bin_number)
        """
        try:
            cursor = self.conn.cursor()
            # Check if bin_assignments exists and has data
            cursor.execute("PRAGMA table_info(bin_assignments)")
            if cursor.fetchall():
                cursor.execute("SELECT COUNT(*) FROM bin_assignments")
                count = cursor.fetchone()[0]
                if count > 0:
                    logger.info("Migrating existing bin_assignments table due to schema change")
                    cursor.execute("DROP TABLE bin_assignments")
                    self.conn.commit()

            cursor.execute("""
                CREATE TABLE IF NOT EXISTS bidders_old (
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    # Section: subscriptions.db schema creation / verification
    # ──────────────────────────────────────────────────────────────────────────────

    def _initialize_subscription_tables(self):
        """
        Create or verify the subscription tables in subscriptions.db:
          - subscriptions(email, tier, license_key)
          - settings(email, chat_id, top_buyer_text, giveaway_announcement_text, flash_sale_announcement_text, multi_buyer_mode)
        """
        try:
            ...
            logger.info("Ensured subscriptions & settings tables exist")
        except sqlite3.Error as e:
            logger.error("Failed to initialize subscriptions/settings tables: %s", e)
            raise

    # ──────────────────────────────────────────────────────────────────────────────
    # Section: Bidder logic (operates on bidders.db)
    # ──────────────────────────────────────────────────────────────────────────────

    def assign_bin(self, username):
        """
        Assign a bin to 'username' in bidders.db.
        Returns the assigned bin number.
        """
        ...
        try:
            ...
            # Check if username already has a bin
    ...
    def get_bin_count(self):
        """
        Return the total number of unique bins assigned so far (count of rows in bin_assignments).
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM bin_assignments;")
            count = cursor.fetchone()[0] or 0
            logger.debug("Retrieved total bin count: %d", count)
            return count
        except sqlite3.Error as e:
            logger.error("Failed to get bin count: %s", e)
            return 0

    def add_transaction(self, username, original_username, qty, weight, is_giveaway):
        """
        Add a bidder transaction to bidders_old (either a regular bin assignment or a giveaway).
        Returns (bin_number, giveaway_number).
        """
        if not username or not isinstance(username, str):
            raise ValueError("Username must be a non-empty string")
        ...
        try:
            ...
            # Normalize username to lowercase for DB
            uname = username.strip().lower()
            ...
            # Insert into bidders_old
            cursor = self.conn.cursor()
            cursor.execute("""
                INSERT INTO bidders_old (username, original_username, quantity, weight, is_giveaway, bin_number, giveaway_number, timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (uname, original_username, qty, weight, int(is_giveaway), bin_num, giveaway_num, timestamp))
            ...
            logger.info(
                "Added transaction: %s, qty=%s, giveaway=%s, bin=%s, giveaway_num=%s, timestamp=%s",
                original_username, qty, is_giveaway, bin_num, giveaway_num, timestamp
            )
    ...
    def export_csv(self):
        """
        Export all transactions from bidders_old to a CSV file. Returns the CSV path.
        """
        headers = [
            'username',
            'original_username',
        ...
        try:
            ...
            cursor = self.conn.cursor()
            cursor.execute(f"""
                SELECT username, original_username, quantity, weight, is_giveaway, bin_number, giveaway_number, timestamp
                FROM bidders_old
            """)
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writeheader()  # don’t forget the header row!
                for row in rows:
                    # convert tuple to dict[string→value]
    ...
    def import_csv(self, file_path):
        """
        Import bidder data from a CSV into bidders_old. Expected headers (case-insensitive):
          - Required: username (or user, name), quantity (or qty, count)
          - Optional: original_username (or display_name, original_name; defaults to username),
                      weight, is_giveaway, bin_number, giveaway_number, timestamp
        """
        try:
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        cursor.execute("""
                            INSERT INTO bidders_old (username, original_username, quantity, weight, is_giveaway, bin_number, giveaway_number, timestamp)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (uname, orig_uname, qty, weight, is_giveaway, bin_num, giveaway_num, timestamp))
    ...
    def start_show(self):
        """
        Mark the show start time in memory and test the bidders DB connection.
        """
        try:
    ...
    def get_avg_sell_rate(self):
        """
        Compute sell rate from bidders_old. Returns (per_hour, per_minute, 2h, 3h, 4h projections).
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("""
            ...
            try:
                min_time = datetime.strptime(min_ts, "%Y-%m-%d %H:%M:%S")
                max_time = datetime.strptime(max_ts, "%Y-%m-%d %H:%M:%S")
    ...
    def clear_bidders(self):
        """
        Remove all transactions and bin_assignments in bidders.db,
        and reset in-memory counters.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("DELETE FROM bidders_old;")
            ...
            self.show_start_time = None
            self.bidders.clear()
            logger.info("Cleared all bidders from bidders.db")
    ...
    def get_top_buyers(self):
        """
        Return top 5 buyers (username, quantity, bin_number) from bidders_old.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("""
                SELECT original_username, SUM(quantity), MAX(bin_number)
                FROM bidders_old
                WHERE is_giveaway = 0
                GROUP BY username
    ...
    def print_bidders(self):
        """
        Return transactions from bidders_old as a list of tuples:
        (original_username, quantity, bin_number, giveaway_number, weight, timestamp).
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("""
                SELECT original_username, quantity, bin_number, giveaway_number, weight, timestamp
                FROM bidders_old
                ORDER BY timestamp DESC
            """)
            transactions = cursor.fetchall()
            logger.debug("Retrieved %d transactions from bidders_old", len(transactions))

            # Update in-memory cache
            self.bidders.clear()
            for trans in transactions:
                orig_uname, qty, bin_num, giveaway_num, weight, timestamp = trans
            ...
            logger.error("Failed to retrieve transactions from bidders_old: %s", e)
            return []

    # ──────────────────────────────────────────────────────────────────────────────
    # Section: subscription lookups (operates on subscriptions.db)
    # ──────────────────────────────────────────────────────────────────────────────

    def get_user_tier(self, user_email):
        """
        Fetch the current tier for user_email from subscriptions.db.
        Returns None if no subscription record.
        """
        if not user_email:
            return None
    ...
    def get_user_license_key(self, user_email):
        """
        Return the license_key (Stripe ID) for user_email from subscriptions.db.
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    # Section: close both connections
    # ──────────────────────────────────────────────────────────────────────────────
